...
export class MaterialPersianDateAdapter extends DateAdapter<jalaliMoment.Moment> {
  ...
  getDayOfWeekNames(style: "long" | "short" | "narrow"): string[] {
    switch (style) {
      ...
      case "narrow":
        return ["یک شنبه", "دوشنبه", "سه‌شنبه", "چهار‌شنبه", "پنج‌شنبه", "جمعه", "شنبه"];
    }
  }
  ...
  createDate(year: number, month: number, date: number): jalaliMoment.Moment {
    ...
    if (date < 1) {
      throw Error(`روز "${date}". مجاز نیست.`);
    }
    ...
    if (this.getMonth(result) !== month) {
      throw Error(`تاریخ غیرمجاز ${date} برای ماه ${month} ام.`);
    }
    if (!result.isValid()) {
      throw Error(`تاریخ غیرمجاز "${date}" برای ماه "${month}" ام.`);
    }
  ...
  format(date: jalaliMoment.Moment, displayFormat: string): string {
    ...
    if (!this.isValid(date)) {
      throw Error("رشته ی وارده یک تاریخ نیست.");
    }
